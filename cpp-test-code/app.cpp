...
extern "C"
int host_dlopen(const char* path, int flags)
    __attribute__((import_module("host"), import_name("host_dlopen")));
...
//        std::printf("acc is %d\n", acc);
        ...
        printf("host_dlopen called without errors and the returned handle is %d!\n", host_dlopen(library_name1, safe_size_t_to_int(std::strlen(library_name1))));

        const char *library_name2 = "mult.wasm";
        printf("host_dlopen called without errors and the returned handle is %d!\n", host_dlopen(library_name2, safe_size_t_to_int(std::strlen(library_name2))));

        const char *library_name3 = "mult2.wasm";
        printf("host_dlopen called without errors and the returned handle is %d!\n", host_dlopen(library_name3, safe_size_t_to_int(std::strlen(library_name3))));
