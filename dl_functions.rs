...
pub fn make_wasm_dlopen(mut store: impl AsContextMut<Data = WasiP1Ctx>) -> Func {
    ...
    return Func::wrap(
        ...
        |mut caller: Caller<'_, WasiP1Ctx>, ptr: i32, library_len: i32| -> i32 {
            ...
            let end = Instant::now();   // Record the end time                 // TODO we need to first instantiate its requirements
            let compile_duration = end.duration_since(start);
            let _ = write_timing_to_file("Compile duration was: ", compile_duration.as_millis(), "ms\n");


            let start = Instant::now();
            let instance = linker.instantiate(store, &module).unwrap();
            let end = Instant::now();
            let instantiation_duration = end.duration_since(start);
            let _ = write_timing_to_file("Instantiation duration was: ",instantiation_duration.as_millis(), "ms\n");
            let _ = write_timing_to_file("Compile + Instantiation duration was: ", instantiation_duration.add(compile_duration).as_millis(), "ms\n");
...
pub fn write_timing_to_file<T: Display>(prefix:&str ,content: T, suffix: &str) -> io::Result<()> {
    let filename = "/Users/dragonulq/Repos/WebAssemblyFinalProject/cpython/cross-build/wasm32-wasip1/timings_instance_not_alive.txt".to_string();
